...
class GzipDisk(Disk):
    def store(self, value, read, key=None):
        ...
        :param value: value to convert 변환할 값
        :param bool read: True when value is file-like object 파일 값이 유사일 경우 참
        :return: (size, mode, filename, value) tuple for Cache table 캐시 테이블의 튜플(크기, 모드, 파일이름, 값)
        """
        ...
        if type(value) is BytesType:
            ...
            str_io = BytesIO()
            gz_file = gzip.GzipFile(mode='wb', compresslevel=1, fileobj=str_io) # 1은 가장 빠르고 압축률이 낮음
...
def getCache(scope_str):
    return FanoutCache('data-unversioned/cache/' + scope_str, # data-unversioned
                       disk=GzipDisk,
                       shards=64,
                       timeout=1,
                       size_limit=3e11,
