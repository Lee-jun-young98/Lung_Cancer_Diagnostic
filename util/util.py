...
def irc2xyz(coord_irc, origin_xyz, vxSize_xyz, direction_a):
    cri_a = np.array(coord_irc)[::-1] # 넘파이 배열로 변환하여 순서를 바꿈
    origin_a = np.array(origin_xyz)
    vxSize_a = np.array(vxSize_xyz)
    coords_xyz = (direction_a @ (cri_a * vxSize_a)) + origin_a # 2 3 4 단계를 한번에 실행
...
def xyz2irc(coord_xyz, origin_xyz, vxSize_xyz, direction_a):
    ...
    cri_a = ((coord_a - origin_a) @ np.linalg.inv(direction_a)) / vxSize_a # 4, 3, 2  단계를 실행 np.linalg.inv 역행렬 계산
    cri_a = np.round(cri_a) # 정수로 변환하기 전에 적절히 반올림
